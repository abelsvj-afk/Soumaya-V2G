...
/**
 * Lenses client (revived, docs/overworld/lenses-revival.md, task #72) — saved, deterministic
 * queries over the memory graph. Thin wrappers over /api/lenses; all space-scoped server-side
 * via the x-space-id header. The server route/repo/shared types were never touched by the
 * Overworld rewrite — only this client file was deleted, leaving the feature reachable again by
 * recreating it verbatim (same signatures, same safe-fallback shape) rather than redesigning it.
 */
...
/** Evaluate a lens → the real node ids the Library should filter its shelves to. */
