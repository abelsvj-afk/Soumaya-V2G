...
- Direct response to: "building interiors are like 75% off screen for some reason when you enter a
  ...
  beginnning needs to be possible. Just like city builder games you start earn right away as soon
  as you start placing buildings and have people coming into the town." Resolved in
  `docs/overworld/interior-camera-and-income-fixes.md`, per Rule #1.
- **Camera bug** — measured directly (real Phaser camera-clamp math against real
  ...
  one shared camera-bounds rectangle set once at scene creation.
- Fixed with real dynamic bounds toggling: new `interiorRoomBounds()` (`interiorRoom.ts`,
  replacing `worldBoundsTiles()`), new `applyInteriorCamera()`/`restoreExteriorCamera()`
  (`ExteriorScene.ts`) computing a real "contain" fit-zoom, applied synchronously at the START of
  `enterInterior()` so even the entry tween isn't seen through stale bounds. Measured post-fix:
  29.9%-63.3% screen coverage across the same 3 viewports.
- **No real-looking interior** — fixed by painting the real floor with `TileFrame.path` (already-
  loaded outdoor plaza tile, zero new sourcing) and retoning the wall to the real
  `uiColor.fieldBorder` design token.
- **Earning right away** — measured the original `passiveIncome.ts` rate: a representative 500¢
  structure took over 2 real hours to earn a single whole cent. Retuned `DAILY_RATE: 0.1 → 1.2`
  with `MAX_ACCRUAL_DAYS: 3 → 1` — the same structure now earns within single-digit real minutes.
- **Population income** — new `data/passiveResidentIncome.ts` closes "people coming into the town"
  earning nothing: 1 real cent per real hour for every Resident (task #118) currently, genuinely
  HOUSED per `housing.ts`'s real `assignResidents()`, capped at 24 real hours per call, same
  baseline/no-retroactive-payout convention as `passiveNpcIncome.ts`. All three passive-income
  streams (structure rent, NPC working-hours, Resident housing) now stack independently.
- Verified by pre/post-fix camera-clamp measurements, 1 new + 4 updated `passiveIncome.test.ts`
  cases, 1 new + 1 updated `interiorRoom.test.ts` case, 7 new `passiveResidentIncome.test.ts`
  cases, and the full gate (1066 server + 646 web tests, typecheck, build). Not yet seen rendered
  in a real browser — the camera/zoom fix especially needs on-device reconfirmation.
...
- A real Mayor's Office "how's my city doing" view, built entirely from real numbers already
  computed elsewhere in `MayorsHallOverlay.tsx` — never a new invented "happiness %".
- New "Town Report" section at the top: Population, Treasury, Buildings needing a visit (reused
  from the existing Town Health section), Zoning balance (`N residential : M commercial`, or "no
  zoning yet"), Civic concern (`stable` / `a town meeting is due`).
- Added one new read-only getter, `civicConcernActive(spaceId)` (`civicConcern.ts`) — the
  existing `checkCivicConcern` writes as a side effect of evaluating a NEW check, which the Town
  Report shouldn't trigger just by being viewed.
- Deliberately deferred: building tiers from real accumulated use (§E part 2) — the real neglect
  data stored is a single timestamp, not a continuous history, so this needs either a new streak-
  tracking mechanism or a documented proxy, a design decision not guessed this round.
- Verified by 4 new `civicConcern.test.ts` cases + 5 new `MayorsHallOverlay.test.tsx` cases + the
  full gate (1066 server + 592 web tests, typecheck, build). Not yet seen rendered in a real
  browser.
...
- Direct user request, reversing an earlier deprioritization. See
  `docs/overworld/mindspace.md` for the full account.
- The data (`getThoughts()`, real live working-memory list `SanctuaryOverlay.tsx` already
  manages) was already real; the gap was purely presentational — never visible outside that one
  building.
- Resolved design: motes are read-only ambient decoration only (no second interaction surface —
  managing a thought stays exclusively in the Sanctuary), and orbit the PLAYER rather than any
  fixed world tile, since a thought has no real location.
- New pure module `adapter/moteLayout.ts` (`moteOffset`, `strongestThoughts`) — deterministic
  circular drift per thought hashed from its id, frozen under `prefersReducedMotion()`.
- Capped at the top 6 by strength; alpha carries strength, scale carries reinforceCount — two
  independent non-color cues.
- `WorldSnapshot` gained a `thoughts` field (fetched in `loadWorldSnapshot()`);
  `ExteriorScene.setThoughts()` mirrors `setCreatures()`'s existing pending/render pattern;
  `update()` repositions every mote each frame from the player's current position.
- Deferred: NPC awareness/commentary on the motes (a distinct enhancement idea, not the actual
  ask).
